...
(see Section~\ref{sec:discussion} for a more detailed discussion of these points
...
In Section~\ref{sec:sims} we describe a suite of $N$-body simulations that span a
range of progenitor masses on a characteristic, mildly eccentric orbit and then
use them in Section~\ref{sec:method} to motivate a new, flexible model for tidal
streams that works entirely in phase-space. In Section~\ref{sec:experiments} we
...
of data from $N$-body simulations. In Section~\ref{sec:discussion}, we discuss the
results of these experiments and the extent to which we address the above points
1-5. We conclude in Section~\ref{sec:conclusion}.
...
potential forms. Figure~\ref{fig:potential} shows equipotential slices in the
...
from the final time-step of four $N$-body simulations (Section~\ref{sec:sims})
...
Section~\ref{sec:experiments}.}\label{fig:sims}
\end{center}
\end{figure*}
...
duration of the simulations of Section~\ref{sec:sims}.
...
of stars from one of the $N$-body simulations described in Section~\ref{sec:sims}.
...
\begin{center}
    ...
    Section~\ref{sec:experiments}. $\mathcal{N}$ is the normal (Gaussian)
...
$2.5\times10^8\,\msun$ simulation (Section~\ref{sec:sims}), assuming the
...
\caption{ Positions of all 64 MCMC walkers (see Section~\ref{sec:experiments})
...
(Section~\ref{sec:exp2}). Axis ranges show the lower and upper bounds on the
...
(Section~\ref{sec:exp2}). }\label{fig:exp2_satellite}
\end{center}
\end{figure*}
...
(Section~\ref{sec:exp2}).   }\label{fig:exp2_particle}
\end{center}
\end{figure*}
...
As with Section~\ref{sec:exp2}, this experiment includes 41 model parameters in
